...
class TransformSamplingSubTraj:
    ...
    def __call__(self, traj):
        ...
        #inputs_n["observations"] = traj["observationsold"][si: si + self.max_len, :, :, :, :]
        ...
        if tlen != act_len:
            raise ValueError
        #/scratch/nstevia/torchbeast_ppo_pacman/logs/torchbeast/torchbeast-20230724-054854/
        ...
        dd = torch.from_numpy(dd).to(dtype=torch.bool)
        dones = torch.from_numpy(dones).to(dtype=torch.bool).reshape(-1)
        ...
        return ss, aa, rr, dones, rtg, timesteps, ordering, padding_mask,inputs


def collate_fn(batch):
    # print(len(batch))
    inp = {'actions': [], 'observations': [], 'rewards': [], 'returns-to-go': [], 'actlogprob': [], 'stateval': []}
    ...
    return None, None, None, don, None, None, None, None, inputs
