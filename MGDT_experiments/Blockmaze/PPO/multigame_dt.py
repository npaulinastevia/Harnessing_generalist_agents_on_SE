...
class MultiGameDecisionTransformer(nn.Module):
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.randn(19 * num_steps, self.d_model)).cuda() #nn.Parameter(torch.randn(tokens_per_step * num_steps, self.d_model))to(device=device)


        self.ret_linear = nn.Linear(self.d_model, self.num_returns)
        if stochastic_policy:
            self.act_linear=DiagGaussianActor(self.d_model, self.num_actions)
            self.act_linear1 = nn.Linear(self.d_model, 4)
    ...
    def _image_embedding(self, image: Tensor):
        ...
        image = image.to(dtype=torch.float32) / 1.0 #image.to(dtype=torch.float32) / 255.0
        ...
        image_pos_enc = nn.Parameter(torch.randn(1, 1, 16, self.d_model)).cuda() #nn.Parameter(torch.randn(1, 1, 16, self.d_model)).to(device=device)
    ...
    def get_predictions(
        ...
        states = states.reshape(num_envs, -1, self.state_dim[0],self.state_dim[1])
                ...
                [
                    torch.zeros(
                        (
                            ...
                            self.state_dim[0],self.state_dim[1],
                        ),
        ...
        inputs["returns-to-go"]=returns_to_go
        inputs["rewards"]=rewards


        r = self.forward(inputs)  #state_preds, action_preds, return_preds = self.forward(inputs)
        state_preds=None
        action_preds=r['action_logits']
        return_preds=r['return_logits']
        if self.stochastic_policy:
            return state_preds, action_preds, return_preds[:, -1] #state_preds[:, -1], action_preds, return_preds[:, -1]
        else:
            return (
                state_preds,
                self.clamp_action(action_preds[:, -1]),
                return_preds[:, -1],
            )  #(state_preds[:, -1],self.clamp_action(action_preds[:, -1]),return_preds[:, -1],)
    ...
    def optimal_action(
        ...
        intermediate=logits_fn(inputs)
        act_logits = intermediate["action_logits"][:, timestep, :] #intermediate["action_logits"][:, timestep, :]
        ( act_sample,_,_ )= sample_from_logits(
            act_logits,
            generator=rng,
            deterministic=deterministic,
        ...
        act_val = intermediate["act_val"]
        (_,logts,_)=sample_from_logits(
            intermediate["action_logits"],
            generator=rng,
            deterministic=deterministic,
            temperature=action_temperature,
            top_percentile=action_top_percentile,
