...
class MultiGameDecisionTransformer(nn.Module):
    ...
    def forward(self, inputs: Mapping[str, Tensor]) -> Mapping[str, Tensor]:
        ...
        # Embed inputs.
        print(inputs["observations"].shape,inputs["returns-to-go"].shape,inputs["actions"].shape,inputs["rewards"].shape) #torch.Size([1, 4, 1, 84, 84]) torch.Size([1, 4]) torch.Size([1, 4]) torch.Size([1, 4])
