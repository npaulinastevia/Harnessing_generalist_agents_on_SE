        ...
        while True:  # for t in range(num_steps)
            ...
            if mode == 'eval':
                if step_results[0][3]['bug'] is not None:
                    #if step_results[0][3]['bug'] not in buglist:
                        if step_results[0][3]['bug'] not in lebug:
                            numberbugs = numberbugs + 1
                            #print('number of bugs', numberbugs)
                            lebug.append(step_results[0][3]['bug'])
                            fi = open(str(filei)+'typeofbugs10k.txt', 'a+')
