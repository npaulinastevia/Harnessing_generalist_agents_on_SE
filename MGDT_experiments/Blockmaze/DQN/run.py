...
class Experiment:
    def __init__(self, variant,parser):
        ...
        self.f = open(str(self.variant["filei"])+'bug_log_RELINE10k.txt', 'w')
        self.f.close()
        ...
        fi = open(str(self.variant["filei"])+'bug_log10k.txt', 'a+')
        fi.write('episodes,numberofbugs,mean_reward,loss,time,timeeval'+ os.linesep)
        fi.close()
        fi = open(str(self.variant["filei"])+'typeofbugs10k.txt', 'a+')
    ...
    def _load_model_pretrain(self, path_prefix):
        path_prefix='/scratch/nstevia/online-dt-wuji-dqn1/exp/2023.08.13/144709-default'
        if Path(f"{path_prefix}/model.pt").exists():
            with open(f"{path_prefix}/model.pt", "rb") as f:
    ...
    def evaluate(self,filei): #def evaluate(self, eval_fns):
    ...
    def online_tuning(self, online_envs, eval_envs, loss_fn):
        ...
        while self.online_iter < maxon:#self.variant["max_online_iters"]:
            ...
            if not evaluate:
                ...
                with open(str(self.variant["filei"])+"myfile_loss10k.txt", 'a+') as f:
        ...
        while self.online_iter < maxon:#self.variant["max_online_iters"]:
            ...
            outputs = {}
            if evaluate:
                out, x = self.evaluate(self.variant["filei"])
            if not evaluate:
                x=self._augment_trajectories(
            ...
            if not evaluate:
                ...
                with open(str(self.variant["filei"])+"myfile_loss10k.txt", 'a+') as f:
                    for key, value in train_outputs.items():
                        f.write('%s:%s\n' % (key, value))
            fi = open(str(str(self.variant["filei"]))+'bug_log10k_test.txt', 'a+')
            fi.write(str(self.online_iter)+','+str(self.numberOfbugs)+','+str(self.r)+','+str(None)+','+str(str(millis_interval(start, datetime.datetime.now())/1000))+','+str(out["time/evaluation"])+ os.linesep)
            fi.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filei", type=int, default=0)
