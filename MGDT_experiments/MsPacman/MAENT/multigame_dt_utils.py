...
def accuracy(logits, labels):
    ...
    acc = torch.eq(predicted_label, labels.view(predicted_label.shape)).to(dtype=torch.float32)
...
def sample_from_logits2(
    ...
    return (sample,D.Categorical(logits=temperature * logits).log_prob(sample),D.Categorical(logits=temperature * logits).entropy())
