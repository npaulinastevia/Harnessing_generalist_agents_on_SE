...
class MultiGameDecisionTransformer(nn.Module):
    def __init__(
        ...
        if stochastic_policy:
            ...
            self.act_linear1 = DiagGaussianActor(self.d_model, 5)
        else:
            self.act_linear = nn.Linear(self.d_model, self.num_actions)
            self.act_linear1 = nn.Linear(self.d_model, 5)
        if self.predict_reward:
    ...
    def forward(self, inputs: Mapping[str, Tensor]) -> Mapping[str, Tensor]:
        ...
        token_emb=token_emb.to(device=self.dev1)
        mask = mask.to(device=self.dev1)
        custom_causal_mask = custom_causal_mask.to(device=self.dev1)
        output_emb = self.transformer(token_emb, mask, custom_causal_mask)

        output_emb=output_emb.to(device=self.dev0)
        token_emb=token_emb.to(device=self.dev0)
        mask = mask.to(device=self.dev0)
        custom_causal_mask = custom_causal_mask.to(device=self.dev0)
        ...
        #act_linear = nn.Linear(self.d_model, 5,device=act_pred.device)
