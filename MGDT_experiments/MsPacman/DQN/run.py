...
class Experiment:
    def __init__(self, variant,parser):
        ...
        self.f = open('4bug_log_RELINE2gpu10k.txt', 'a+')
        self.f.close()
    ...
    def _load_model_pretrain(self, path_prefix):
        path_prefix='./exp/2023.11.19/061656-default'
        if Path(f"{path_prefix}/model.pt").exists():
            with open(f"{path_prefix}/model.pt", "rb") as f:
                checkpoint_pretrain = torch.load(f,map_location=lambda storage, loc: storage)
    ...
    def _load_model(self, path_prefix):
        path_prefix = '/scratch/nstevia/muldtdqnpacman/exp/2023.09.29/015420-default'
    ...
    def _load_dataset(self, env_name):
        def _batch_rollout(envs, policy_fn, num_episodes, log_interval=None):
            ...
            for c in range(num_episodes // num_batch):
                o, a, r, d,rtg,oold,aold,rold,dold = [], [], [], [],[], [], [], [],[]
                seeds = seeds_list[c * num_batch: (c + 1) * num_batch]
                rng.manual_seed(seeds[0])
                ...
                traj = {'observations': np.array([]), 'actions2': np.array([]), 'rewards': np.array([]),
                        'terminals': np.array([]),'rewards2': np.array([]),'actions': np.array([]),"returns-to-go": np.array([]),'observationsold': np.array([]), 'actions2old': np.array([]), 'rewardsold': np.array([]),
                        'terminalsold': np.array([]),'rewards2old': np.array([]),'actionsold': np.array([]),"returns-to-goold": np.array([])}
                #traj['actions'] = np.append(traj['actions'], 0)
                #traj['rewards'] = np.append(traj['rewards'], 0)
                
                traj0 = {'observationsold': np.array([]), 'actions2old': np.array([]), 'rewardsold': np.array([]),
                        'terminalsold': np.array([]),'rewards2old': np.array([]),'actionsold': np.array([]),"returns-to-goold": np.array([]),}
                #traj0['actions'] = np.append(traj0['actions'], 0)
                #traj0['rewards'] = np.append(traj0['rewards'], 0)

                for t in range(num_steps):
                    epsilon = EPS_END + (EPS_START - EPS_END) * \
                                    math.exp(-1. * t / EPS_DECAY)
                    done_prev = done
                    oold.append(obs['observations'][0])
                    aold.append(obs['actions'][0])
                    rold.append(obs['rewards'][0])
                    ...
                    if np.random.random() < epsilon:
                        ...
                        rg = numpy.zeros_like(obs['actions'].cpu().numpy())  # .cpu().numpy())
                        rtg.append(rg[0])
                    else:
    ...
    def online_tuning(self, online_envs, eval_envs, loss_fn):
        ...
        #dist.init_process_group(backend=self.variant["dist_backend"], init_method=self.variant["init_method"], world_size=self.world_size,
         #                       rank=self.rank)
        self.dev0 = self.rank * 2
        self.dev1 = self.rank * 2 + 1
        self.dev0 = self.dev1 = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        ...
        self.target_net = self.target_net.to(device=self.dev0)
        ...
        self.target_net.transformer = self.target_net.transformer.to(device=self.dev1)
        self.model.transformer = self.model.transformer.to(device=self.dev1)
        #self.model = DDP(self.model,find_unused_parameters=True)#,find_unused_parameters=True
        #self.model.module.transformer = DDP(self.model.module.transformer)
        #self.target_net = DDP(self.target_net)
        self.optimizer = Lamb(
        ...
        evaluate=False
        while self.online_iter < 50000000:#self.variant["max_online_iters"]:
           ...
           # outputs.update(augment_outputs)
            ...
            #out=self.evaluate()
            with open("4myfile_loss2gpu10k.txt", 'a+') as f:
    ...
    def __call__(self):
       ...
       # )
        ...
        if self.variant["max_online_iters"]:
            ...
            #)
            online_envs = None
            #run_demo(self.online_tuning(online_envs, eval_envs, loss_fn), self.world_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_to_tb", "-w", type=bool, default=True)#bg12103.int.ets1.calculquebec.ca
    parser.add_argument("--save_dir", type=str, default="./exp")
    parser.add_argument("--exp_name", type=str, default="default")
    parser.add_argument('--init_method', default='tcp://127.0.0.1:51392', type=str, help='')#tcp://127.0.0.1:3456'  tcp://bg12103.int.ets1.calculquebec.ca:3456
