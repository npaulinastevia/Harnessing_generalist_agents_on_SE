...
class TransformSamplingSubTraj:
    ...
    def __call__(self, traj):
        ...
        # 'mask': (37, 6),
        # 'candidate': (37, 6)}
        dic={x:y.shape for x,y in traj.items()}
        print(dic,'dicc')
        ...
        inputs["adj"] = traj["adj"][si : si + self.max_len,:,:]
        inputs["fea"] = traj["fea"][si: si + self.max_len,:]
        inputs["mask"] = traj["mask"][si: si + self.max_len,:]
        inputs["candidate"] = traj["candidate"][si: si + self.max_len,:]
        inputs['actlogprob']=traj['actlogprob'][si : si + self.max_len]
        inputs['stateval']=traj['stateval'][si : si + self.max_len]
        inputs['idx'] = traj['idx'][si: si + self.max_len]
        inputs_n["actions"]=traj["actions2old"][si : si + self.max_len]
        inputs_n["rewards"] = traj["rewards2old"][si: si + self.max_len]
        inputs_n["returns-to-go"]=traj["returns-to-go"][si : si + self.max_len,:]
        rr = traj["rewards"][si : si + self.max_len].reshape(-1, 1)
        inputs_n["adj"] = traj["adjold"][si : si + self.max_len,:,:]
        inputs_n["fea"] = traj["feaold"][si: si + self.max_len,:]
        inputs_n["mask"] = traj["maskold"][si: si + self.max_len,:]
        inputs_n["candidate"] = traj["candidateold"][si: si + self.max_len,:]
        ...
        #padding_mask = np.concatenate([np.zeros(tlen), np.ones(inputs["actions"].shape[1])]) #np.concatenate([np.zeros(self.max_len - tlen), np.ones(tlen)])
        ...
        return None, aa, rr, dones, rtg, timesteps, ordering, None,inputs,inputs_n


def collate_fn(batch):
  #print(len(batch))
  inp={'actions':[],'observations':[],'rewards':[],'returns-to-go':[],'adj': [], 'fea': [], 'mask': [],
                "candidate": [],"idx": []}
  inps={'actions':[],'observations':[],'rewards':[],'returns-to-go':[],'adj': [], 'fea': [], 'mask': [],
                "candidate": []}
  don=[]
  actions=[]
  observations=[]
  ...
  for x in batch:
      #print(x)
      (ss, aa, rr, dones, rtg, timesteps, ordering, padding_mask, inputs,inputs_n) = x
      ...
      for a in inputs_n:
          ...
          for aa in range(inputs_n[a].shape[0]):
              if len(inputs_n[a].shape) < 2:
                  nonM_n.append(a)

                  inps[a].append(torch.from_numpy(np.array(inputs_n[a][aa])))
                  continue
  ...
  return None, None, None, don,None , None, None, None,inputs,inputs_n
def create_dataloader(
    ...
    return torch.utils.data.DataLoader(
        subset, batch_size=batch_size,collate_fn=collate_fn, num_workers=24, shuffle=False
    )
    #return torch.utils.data.DataLoader(
