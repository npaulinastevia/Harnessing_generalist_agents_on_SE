...
def vec_evaluate_episode_rtg(
    ...
    if mode=='normal':
        for c in range(1):  # num_episodes // num_batch
            o, a, r, d, rtg, adj, fea, mask, cand, st, ac, oold, aold, rold, dold, rtgold, adjold, feaold, maskold, candold, stold, acold, idxx = \
                [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
            ...
            traj = {'observations': np.array([]), 'actions2': np.array([]), 'rewards': np.array([]),
                    ...
                    "candidate": np.array([]), 'stateval': np.array([]), "actlogprob": np.array([]),
                    'actions2old': np.array([]), 'rewardsold': np.array([]),
                    'terminalsold': np.array([]), 'rewards2old': np.array([]), 'actionsold': np.array([]),
                    "returns-to-goold": np.array([]), 'adjold': np.array([]), 'feaold': np.array([]),
                    'maskold': np.array([]),
                    "candidateold": np.array([]), 'statevalold': np.array([]), "actlogprobold": np.array([]),
                    "idx": np.array([])}
            ...
            while True:  # for t in range(num_steps)
                ...
                oold.append(obs['observations'].cpu().numpy()[0])
                aold.append(obs['actions'].cpu().numpy()[0])
                rold.append(obs['rewards'].cpu().numpy()[0])
                # d.append(done_prev[0])
                adjold.append(obs['adj'].cpu().numpy())
                feaold.append(obs['fea'].cpu().numpy())
                maskold.append(obs['mask'].cpu().numpy())
                candold.append(obs['candidate'].cpu().numpy())
                # torch.Size([1, 4, 1, 84, 84]) torch.Size([1, 4]) torch.Size([1, 4]) torch.Size([1, 4])
                if np.random.random() < epsilon:
                    idx = random.randint(0, configs.n_j - 1)
                    actions = select_action_greedy(idx, obs['candidate'])
                    actions = torch.tensor(actions)
                    logprob, act_val = None, None
                    rg = np.zeros_like(obs['actions'].cpu().numpy())  # .cpu().numpy())
                    rtg.append(rg)
                    idxx.append(idx)

                else:
                    actions, idx, _, logprob, rg, act_val = model.optimal_action(obs['fea'], g_pool_step, None, obs['adj'],
                                                                                 obs['candidate'].unsqueeze(0),
                                                                                 obs['mask'].unsqueeze(0), obs,
                                                                                 return_range=RETURN_RANGE,
                                                                                 single_return_token=True,
                                                                                 opt_weight=0,
                                                                                 num_samples=128,
                ...
                if np.all(done) or t == num_steps - 1:
                    ...
                    traj['idx'] = np.stack((p for p in idxx), axis=0)

                    traj['actions2old'] = np.stack((p for p in aold), axis=0)

                    traj['rewards2old'] = np.stack((p for p in rold), axis=0)
                    # traj['statevalold'] = np.stack((p for p in stold), axis=0)
                    # traj['actlogprobold'] = np.stack((p for p in acold), axis=0)

                    traj['adjold'] = np.stack((p for p in adjold), axis=0)

                    traj['feaold'] = np.stack((p for p in feaold), axis=0)
                    traj["maskold"] = np.stack((p for p in maskold), axis=0)
                    traj['candidateold'] = np.stack((p for p in candold), axis=0)
                    trajectories.append(traj)
    ...
    if mode == 'eval':
        ...
        dataLoaded = np.load(
            '/scratch/nstevia/mdgtdqnl2d/DataGen/generatedData' + str(configs.n_j) + '_' + str(
        ...
        for i, data in enumerate(dataset):
            ...
            while True:
                observation = {a: torch.from_numpy(obs[a]).cuda() for a in
                               obs}  # {a: torch.from_numpy(obs[a]).cuda() for a in obs}
                # agent_outputs = model(observation)
                actions, idx, _, logprob, rg, act_val = model.optimal_action(observation['fea'], g_pool_step, None,
                                                                             observation['adj'],
                                                                             observation['candidate'].unsqueeze(0),
                                                                             observation['mask'].unsqueeze(0),
                                                                             observation, return_range=RETURN_RANGE,
                                                                             single_return_token=True,
                                                                             opt_weight=0,
                                                                             num_samples=128,
                                                                             action_temperature=1.0,
                ...
                t = t + 1
                print(done, envs[0].env.max_endTime, rew, envs[0].env.posRewards, -ep_reward + envs[0].env.posRewards,
                      'st')
