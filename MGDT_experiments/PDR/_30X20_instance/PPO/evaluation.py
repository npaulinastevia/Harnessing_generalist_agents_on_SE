...
create_env,
    ...
    if mode=='normal':
        for c in range(1): #num_episodes // num_batch
            o, a, r, d, rtg ,adj,fea,mask,cand,st,ac= [], [], [], [], [],[], [], [], [],[], []
            ...
            while True: #for t in range(num_steps)
                ...
                actions, _, logprob, rg, act_val = model.optimal_action(obs['fea'], g_pool_step, None, obs['adj'], obs['candidate'].unsqueeze(0),
                ...
                ac.append(logprob.detach().cpu().numpy())
    ...
    if mode=='eval':
        ...
        dataLoaded = np.load(
            '/scratch/nstevia/mgdtl2d3020ppo/DataGen/generatedData' + str(configs.n_j) + '_' + str(
        ...
        for i, data in enumerate(dataset):
            ...
            while True:
                observation = {a: torch.from_numpy(obs[a]).cuda() for a in obs}#{a: torch.from_numpy(obs[a]).cuda() for a in obs}
                # agent_outputs = model(observation)
                actions, _, logprob, rg, act_val = model.optimal_action(observation['fea'], g_pool_step, None, observation['adj'],
                                                  observation['candidate'].unsqueeze(0),
                                                  observation['mask'].unsqueeze(0), observation,  return_range=RETURN_RANGE,
            single_return_token=True,
            opt_weight=0,
            num_samples=128,
            action_temperature=1.0,
