...
def learn(
    ...
    for _ in range(15):
        ...
        learner_outputs = {key: tensor for key, tensor in
                           learner_outputs.items()}
        entropy_loss = flags.entropy_cost * compute_entropy_loss(
            learner_outputs["policy_logits"]
        )
        advantages = rewards_p.detach() - learner_outputs['baseline']

        #torch.Size([240]) torch.Size([30, 8]) okaauau
        ratios = torch.exp(learner_outputs["heur"] - batch['heur'].view(-1))
        advantages=advantages.view(-1)
        print(learner_outputs["heur"].shape, batch['heur'].shape, 'okaauau', learner_outputs["policy_logits"].shape,advantages.shape)
        ...
        total_loss = -torch.min(surr1, surr2) + nn.MSELoss()(learner_outputs["baseline"],
                                                             rewards_p) - flags.entropy_cost * torch.mean(
            torch.distributions.Categorical(logits=learner_outputs["policy_logits"]).entropy())
...
def create_buffers(flags, obs_shape, num_actions) -> Buffers:
    ...
    for _ in range(flags.num_buffers):
        for key in buffers:
            #torch.full(size, -1)
            buffers[key].append(torch.full(specs[key]['size'], -1))#buffers[key].append(torch.empty(**specs[key]))
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    checkpointpath1 = flags.save_path1 + '/model.tar'
    ...
    step = step + remain
    print(flags.total_steps, 'ffflflf',step)
    notsave1=True
...
class AtariNet(nn.Module):
    ...
    def forward(self, inputs, core_state=()):
        ...
        if torch.sum(pickeda) > 0:
            ...
            action = torch.distributions.Categorical(action).sample()
        heur = torch.distributions.Categorical(logits=policy_logits).log_prob(action)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path',
                        ...
                        help='Save Path')
    parser.add_argument('--save_path1',
                       default=r'C:\Users\phili\Downloads\10428077\Replication\LTR\Models\AC\Entropy\AspectJ\\',
                       help='Save Path')
    ...
    parser.add_argument('--file_path1', default='1', type=str, help='Project Name')
    ...
    file_path1 = os.path.join(current_directory, '1_f', flags.monobeast + '_LTR')
    ...
    save_path1 = os.path.join(file_path1, 'Models', flags.algo, flags.project_name)
    ...
    Path(file_path1).mkdir(parents=True, exist_ok=True)
    Path(save_path1).mkdir(parents=True, exist_ok=True)
    ...
    flags.file_path1 = file_path1
    flags.save_path1 = save_path1
    main(flags)
