...
def vec_evaluate_episode_rtg(
    ...
    if mode == 'normal':
        for c in range(1):  # num_episodes // num_batch
            o, a, r, d, rtg, adj, fea, mask, cand, topick,oold, aold, rold, dold,topickold = [],[], [], [], [], [], [], [], [], [], [],[], [], [], []
            ...
            traj = {'observations': np.array([]), 'actions2': np.array([]), 'rewards': np.array([]),
                    ...
                    "picked": np.array([]), 'observationsold': np.array([]), 'actions2old': np.array([]),
                'rewardsold': np.array([]),
                'terminalsold': np.array([]), 'rewards2old': np.array([]), 'actionsold': np.array([]),
                "returns-to-goold": np.array([]),"pickedold": np.array([])}
            ...
            while True:  # for t in range(num_steps)
                ...
                temp_action = torch.from_numpy(to_one_hot(pik, max_size=envs[0].env.action_space.n)).to(
                    device).type(
                    torch.float)
                topickold.append(temp_action.cpu().numpy()[0])
                #shape={k:obs[k].shape for k in obs}

                traj['actionsold'] = np.append(traj['actionsold'], obs['actions'][0])
                traj['rewardsold'] = np.append(traj['rewardsold'], np.array([obs['rewards'][0]]))
                ...
                if np.all(done):
                    ...
                    traj["pickedold"] = np.stack((p for p in topickold), axis=0)
