...
from decision_transformer import SquashedNormal
    ...
    def forward(
        ...
        if causal_mask is None:
            ...
            causal_mask = torch.tril(torch.ones((seq_len, seq_len), dtype=torch.bool,
                                                device=device))  # torch.tril(torch.ones((seq_len, seq_len), dtype=torch.bool, device=device))
...
class MultiGameDecisionTransformer(nn.Module):
    def __init__(
        ...
        # num_patches = 16
        self.image_pos_enc = nn.Parameter(
            torch.randn(1, 1, num_patches, self.d_model))  # nn.Parameter(torch.randn(1, 1, num_patches, self.d_model))
        self.image_pos_enc1 = nn.Parameter(torch.randn(1, 1, num_patches, self.d_model))
        self.ret_emb = nn.Embedding(self.num_returns, self.d_model)
        self.act_emb = nn.Embedding(self.num_actions, self.d_model)
        self.act_emb1 = nn.Embedding(self.num_actions, self.d_model)
        if self.predict_reward:
            self.rew_emb = nn.Embedding(self.num_rewards, self.d_model)
            self.rew_emb1 = nn.Embedding(self.num_rewards, self.d_model)
        ...
        self.positional_embedding = nn.Parameter(torch.randn(19 * num_steps,
                                                             self.d_model)).cuda()  # nn.Parameter(torch.randn(tokens_per_step * num_steps, self.d_model))to(device=device)
        self.positional_embedding1 = nn.Parameter(torch.randn(tokens_per_step * num_steps,
                                                              self.d_model))#.cuda()
        self.ret_linear = nn.Linear(self.d_model, self.num_returns)
        if stochastic_policy:
            self.act_linear = DiagGaussianActor(self.d_model, self.num_actions)
            self.act_linear1 = nn.Linear(self.d_model, self.num_actions)
        else:
    ...
    def reset_parameters(self):
        ...
        nn.init.trunc_normal_(self.act_emb1.weight, std=0.02)
        if self.predict_reward:
            nn.init.trunc_normal_(self.rew_emb1.weight, std=0.02)

        nn.init.trunc_normal_(self.positional_embedding1, std=0.02)
        ...
        variance_scaling_(self.act_linear1.weight)
        nn.init.zeros_(self.act_linear1.bias)
    ...
    def _image_embedding(self, image: Tensor):
        ...
        image_emb = self.image_emb1(
            image)#.cuda()  # [BT x D x P x P]   image_emb = self.image_emb(image) emb(image).to(device=device)
        ...
        image_emb = image_emb.view(image_emb.shape[0], 1, -1, self.d_model)
        # image_pos_enc = nn.Parameter(torch.randn(1, 1, 16,
        #                                        self.d_model)).cuda()  # nn.Parameter(torch.randn(1, 1, 16, self.d_model)).to(device=device)
        # image_emb=torch.reshape(image_emb,(1, 4, 16, 1280))

        image_emb = image_emb + self.image_pos_enc1

        return image_emb

    def _embed_inputs(self, obs: Tensor, ret: Tensor, act: Tensor, rew: Tensor) -> Tuple[
        ...
        act_emb = self.act_emb1(act)
        if self.predict_reward:
            rew_emb = self.rew_emb1(rew)
        else:
            rew_emb = None
    ...
    def forward(self, inputs: Mapping[str, Tensor]) -> Mapping[str, Tensor]:
        ...
        num_steps =1# inputs["actions"].shape[0]
        ...
        if self.predict_reward:
            #
            obs_emb = torch.reshape(obs_emb, (-1, obs_emb.shape[-1]))
            ret_emb = torch.reshape(ret_emb, (-1, ret_emb.shape[-1]))
            act_emb = torch.reshape(act_emb, (-1, act_emb.shape[-1]))
            rew_emb = torch.reshape(rew_emb, (-1, rew_emb.shape[-1]))
        ...
        # self.positional_embedding1 = nn.Parameter(torch.randn(tokens_per_step * num_steps, self.d_model)).to(device=device)

        token_emb = token_emb + self.positional_embedding1
        ...
        act_pred1 = self.act_linear1(act_pred)
    ...
    def optimal_action(
        ...
        def ret_sample_fn(rng, logits):
            ...
            (ret_sample,_,_) = sample_from_logits(
                logits,
                generator=rng,
        ...
        act_logits = out["action_logits"][:, timestep, :]
        pickeda = temp_action.view(*act_logits.shape)

        if torch.sum(pickeda) > 0:
            row_sum_gt_zero = torch.sum(F.softmax(act_logits, dim=-1) * pickeda, dim=1) > 0
            if row_sum_gt_zero.all().item():
                action = F.softmax(act_logits, dim=-1) * pickeda
            ...
            action = F.softmax(act_logits, dim=-1)
            action = torch.distributions.Categorical(action).sample()
        ...
        return act_sample,None,logts,inputs["returns-to-go"],act_val
