...
def vec_evaluate_episode_rtg(
    ...
    if mode == 'normal':
        for c in range(1):  # num_episodes // num_batch
            o, a, r, d, rtg, adj, fea, mask, cand, topick,st,ac = [], [], [], [], [], [], [], [], [], [], [], []
            ...
            traj = {'observations': np.array([]), 'actions2': np.array([]), 'rewards': np.array([]),
                    ...
                    "picked": np.array([]), 'stateval': np.array([]),"actlogprob": np.array([])}
            ...
            st.append(np.zeros((1, 1)))
            ac.append(np.zeros(1))
            while True:  # for t in range(num_steps)
                ...
                actions, _, logprob, rg, act_val = model.optimal_action(obs, temp_action, return_range=RETURN_RANGE,
                                                         single_return_token=True,
                                                         opt_weight=0,
                                                         num_samples=128,
                                                         action_temperature=1.0,
                ...
                #print(act_val.shape, logprob.shape ,'evaluation177', actions.item())
