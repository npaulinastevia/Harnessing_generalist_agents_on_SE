...
parser.add_argument("--savedir", default="/scratch/nstevia/100kfinetuning/torchbeastvtracpacman5/logs/torchbeast",
...
def check_bug1(actor_index,flags):
    folder_bug = flags.tmpdir+'/work_stev5/scratch/nstevia/torchbeast_sac_pacman/torchbeast/bug_left/'
    files = os.listdir(folder_bug)
    img_bug = [file for file in files if file.startswith('bug')]
    img = Image.open(flags.tmpdir+'/work_stev5/current_screen'+str(actor_index)+'.png')#flags.tmpdir+'/work_stev5/current_screen'+str(actor_index)+'.png'
    left = 0
    top = 90
    right = 15
    bottom = 120
    im1 = img.crop((left, top, right, bottom))
    im1.save(flags.tmpdir+'/work_stev5/current_test'+str(actor_index)+'.png')#flags.tmpdir+'/work_stev5/current_test'+str(actor_index)+'.png'
    imgA = cv2.imread(flags.tmpdir+'/work_stev5/current_test'+str(actor_index)+'.png')#flags.tmpdir+'/work_stev5/current_test'+str(actor_index)+'.png'
    for elem in img_bug:
...
def check_bug3(actor_index,flags): #flags.tmpdir+'/work_stev7/current_screen'+str(actor_index)+'.png'
    print('hasbeenehere3')
    folder_bug = flags.tmpdir+'/work_stev5/scratch/nstevia/torchbeast_sac_pacman/torchbeast/bug_left/'
    files = os.listdir(folder_bug)
    img_bug = [file for file in files if file.startswith('bug')]
...
def check_bug2(actor_index,flags):
    folder_bug = flags.tmpdir+'/work_stev5/scratch/nstevia/torchbeast_sac_pacman/torchbeast/bug_right/'
    files = os.listdir(folder_bug)
    img_bug = [file for file in files if file.startswith('bug')]
    print('hasbeenehere2')
    img = Image.open(flags.tmpdir+'/work_stev5/current_screen'+str(actor_index)+'.png')#flags.tmpdir+'/work_stev5/current_screen'+str(actor_index)+'.png'
    ...
    try:
        ...
        while True:
            ...
            #while not env_output['done'][0][0]:
                ...
                fi = open('16OPTglstep.txt', 'a+')
                fi.write(str(flags.st) + ',' +str(t)+','+str(actor_index)+','+str(env_output['done'][0][0]) + os.linesep)
                fi.close()
                if st:
                    env.gym_env.env.ale.saveScreenPNG(flags.tmpdir+"/work_stev5/current_screen"+str(actor_index)+".png")#flags.tmpdir+"/work_stev5/current_screen"+str(actor_index)+".png"
                    ...
                    if not bug_flags[3] and check_bug4(actor_index,flags):
                        ...
                        env_output['reward'] = env_output['reward'] + 50
                    if env_output['done'][0][0]:
                        f = open('25OPTbug_log_RELINE_addtrain' + str(actor_index) + '.txt', 'a+')
                        if bug_flags[0]:
                            f.write('BUG1 ')
                        if bug_flags[1]:
                            f.write('BUG2 ')
                ...
                timings.time("step")
                fi = open('25OPToutputrew.txt', 'a+')
                fi.write(str(flags.st) + ',' +str(numberofbug) + ',' + str(env_output['reward'])+ ',' + str(ep) + ',' + str(actor_index)+ os.linesep)
                fi.close()
                if st:
                    fi = open('4testfile.txt', 'a+')
...
def learn(
    ...
    with lock:
        ...
        fi = open('25OPToutputepisodes.txt', 'a+')
        fi.write(str(flags.st) + ',' +
            str('begin') + ',' + str(len(episode_returns)) + ',' + str(tuple(episode_returns.cpu().numpy())) + ',' + str(glstep) + os.linesep)
        fi.close()
        stats = {
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    learner_model = Net(
        env.observation_space.shape, 5, flags.use_lstm
...
class AtariNet(nn.Module):
    ...
    def forward(self, inputs, core_state=()):
        ...
        clipped_reward = torch.clamp(inputs["reward"], -1, 1).view(T * B, 1)
          # 1 1 4 84 84
        #1 1 torch.Size([1, 512]) torch.Size([1, 1]) torch.Size([1, 5]) torch.Size([1, 1])
        ...
        print(policy_logits.shape,baseline.shape,action)
        return (
