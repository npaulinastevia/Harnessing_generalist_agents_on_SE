...
parser.add_argument("--savedir", default="/scratch/nstevia/torchbeastppol2d3020/logs/torchbeast",
...
parser.add_argument("--core", default=6400, type=int, metavar="B",
                    help=".")
parser.add_argument("--unroll_length", default=1, type=int, metavar="T",#80 #40
...
parser.add_argument("--K_epochs", default=15, type=int, metavar="N",
                    ...
                    help="number of opt step for ppo.") #15
...
def act(
    ...
    try:
        ...
        dataLoaded = np.load('/scratch/nstevia/torchbeastppol2d3020/torchbeast/DataGen/generatedData' + str(configs.n_j) + '_' + str(configs.n_m) + '_Seed' + str(
        ...
        while True:
            ...
            #while not env_output['done'][0][0]:
                ...
                timings.time("step")
                fi = open('/scratch/nstevia/torchbeastppol2d3020/18outputrewOPT.txt', 'a+')
...
def learn(
    ...
    with lock:
        ...
        rewards_p = torch.tensor(rewards_p, dtype=torch.float32).to(device=torch.device(configs.device))
 ...
 #{key: tensor[:-1] for key, tensor in learner_outputs.items()}
            ...
            advantages = rewards_p.view(learner_outputs['baseline'].shape).detach() - learner_outputs['baseline']
            print('model1', flags.K_epochs, model.batch_size,batch['adj'].device)
            #batch['adj']=batch['adj'].to(device=torch.device('cpu'))
            #batch['adj'] = batch['adj'].to_dense()
            #batch['adj'] = batch['adj'].to(device=device)
            entropy_loss = flags.entropy_cost * compute_entropy_loss(
                learner_outputs["pol_log"]
            )



            #print(batch['heur'].shape, learner_outputs['heur'].shape, 'shape')
            ratios = torch.exp(learner_outputs["heur"].view(batch['heur'].shape) - batch['heur'])

            surr1 = ratios * advantages.view(ratios.shape)
            surr2 = torch.clamp(ratios, 1 - flags.eps_clip, 1 + flags.eps_clip) * advantages.view(ratios.shape)
            #print(torch.min(surr1, surr2).shape,D.Categorical(logits=learner_outputs["policy_logits"]).entropy().shape,learner_outputs["baseline"].shape)
            total_loss = -torch.min(surr1, surr2) + nn.MSELoss()(learner_outputs["baseline"].view(ratios.shape),
                                                           rewards_p.view(ratios.shape)) - flags.entropy_cost * torch.mean(D.Categorical(logits=learner_outputs["policy_logits"]).entropy())

            episode_returns = batch["episode_return"][batch["done"]]
            fi = open('/scratch/nstevia/torchbeastppol2d3020/8outputepisodes.txt', 'a+')
            fi.write(
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    checkpoint_pretrain = torch.load('/scratch/nstevia/torchbeastppol2d3020/palaas/torchbeast/torchbeast-20230515-132901/model.tar',map_location='cpu')
    ...
    checkpoint()
    fi = open('/scratch/nstevia/torchbeastppol2d3020/18traintimeOPT.txt', 'a+')
    ...
    dataLoaded = np.load(
        '/scratch/nstevia/torchbeastppol2d3020/torchbeast/DataGen/generatedData' + str(configs.n_j) + '_' + str(
    ...
    for i, data in enumerate(dataset):
        ...
        while True:
            ...
            fi = open('/scratch/nstevia/torchbeastppol2d3020/7testresults'+str(configs.n_j)+str(configs.n_m)+'.txt', 'a+')
...
class AtariNet(nn.Module):
    ...
    def forward(self, x,graph_pool,
        ...
        #print(x.shape,adj.shape,candidate.shape,mask.shape,'500')#torch.Size([1000, 2]) torch.Size([1000, 1000]) torch.Size([1, 50]) torch.float32 torch.int32 torch.int32 torch.bool
        ...
        if self.use_lstm:
            ...
            #print('core', core_input.shape, core_state[0].shape)
            for input, nd in zip(core_input.unbind(), notdone.unbind()):
                # Reset core state to zero whenever an episode ended.
                # Make `done` broadcastable with (num_layers, B, hidden_size)
                # states:

                nd = nd.view(-1, 1, 1)#nd.view(1, -1, 1)
        ...
        return (
            dict(policy_logits=torch.squeeze(pi), baseline=torch.squeeze(baseline), action=action,action_P=action_P
                 , pol_log=torch.squeeze(policy_logits),heur=heur),
            list(core_state),
        )
