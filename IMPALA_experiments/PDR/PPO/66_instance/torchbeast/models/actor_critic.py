...
from torchbeast.models.mlp import MLPActor
from torchbeast.models.mlp import MLPCritic
...
class ActorCritic(nn.Module):
    ...
    def forward(self,
        ...
        h_pooled, h_nodes = self.feature_extract(x=x,
                                                 ...
                                                 adj=adj)
        #print(self.n_j,x.shape,padded_nei,graph_pool.shape,adj.shape,self.n_j,candidate.shape,h_nodes.shape)
        #torch.Size([1000, 2]) None torch.Size([1, 1000]) torch.Size([1000, 1000]) torch.Size([1, 50]) torch.Size([1000, 64])
        ...
        print(concateFea.shape,'3') #torch.Size([1, 50, 128])

        candidate_scores = self.actor(concateFea)
        print(candidate_scores.shape,h_pooled.shape,'2')#torch.Size([1, 50, 1]) torch.Size([1, 64])
        ...
        #print(pi.shape,'1')#torch.Size([1, 50, 1])
        v = self.critic(h_pooled)
        #print(h_pooled.shape,v.shape)
